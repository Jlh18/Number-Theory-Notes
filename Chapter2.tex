...
\begin{dfn}[$p$-adic valuation]
    ...
    sending any non-zero $x$ to $v_p (x) = n$ 
    where $n$ is the unique integer
    and $u$ the unique unit such that $x = p^n u$,
    and sending $0$ to $\infty$.
\end{dfn}
...
\begin{proof}
    ...
    Call the nieghbourhood filter for a point $a$ 
    in the metric tolopogy $N_0(a)$
    and the other $N_1(a)$.
    We use $\<\star \st \dots\>$ to mean the nieghbourhood filter generated
    by $\star$.
    \begin{align*}
        N_1(a) &= 
        ...
        &= N_0(a)
    \end{align*}
    The last part comes from the observation that
